...
fn main() {
    ...
    if let Some(config) = config {
        ...
        let python_version = version::get_python_version(&config.package, &config.dir_path);
        println!("Python version: {}", python_version);

        // Verify pyenv version
        pyenv::verify(&config.package, &python_version, &config.dir_path);

        // Check for virtual environment directories
        venv::verify(&config.package, &config.dir_path);
    }
}
